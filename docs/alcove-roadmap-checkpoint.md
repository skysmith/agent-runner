...
- **Studio modes**
  artifact-driven workspaces such as Game Studio, Web Studio, Data Studio, Docs Studio, Image Studio, and Video Studio
...
- Added Image Studio as the authoritative image-generation workflow inside Alcove instead of relying on a separate dashboard
- Added native image uploads, queued image runs, and source-image-to-3D lineage scaffolding under the Image Studio workspace model
- Added Video Studio entry points in the same Alcove shell so future media workflows do not need a separate product surface
...
- Studio Platform:
  ...
  - artifact-visible studio proof via Game, Web, Data, Docs, Image, and Video shells
- Desktop runtime:
...
- keep Image Studio clearly documented as the supported image-generation path
...
### 5. Media backend follow-through

- wire the real image backend through the existing Image Studio provider boundary
- keep `Make 3D` behind the current provider contract rather than introducing a separate 3D surface
- connect Video Studio to a worker-backed runtime without changing the Alcove-owned workflow
