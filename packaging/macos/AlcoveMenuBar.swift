...
final class MenuBarController: NSObject, NSApplicationDelegate {
    ...
    private let openImageStudioItem = NSMenuItem(title: "Open Image Studio", action: #selector(openImageStudio), keyEquivalent: "")
    private let launchArcadeItem = NSMenuItem(title: "Arcade", action: #selector(launchArcade), keyEquivalent: "")
    ...
    private func configureMenu() {
        ...
        menu.addItem(openImageStudioItem)
        menu.addItem(launchArcadeItem)
        ...
        for item in [openItem, openImageStudioItem, launchArcadeItem, openWorkspaceItem, stopItem, copyLocalItem, copyPhoneItem, restartItem, quitItem] {
            item.target = self
        }
    ...
    private func updateMenu() {
        ...
        openImageStudioItem.isEnabled = state.binaryPath != nil || state.localURL != nil
        launchArcadeItem.isEnabled = state.binaryPath != nil
    ...
    @objc private func openImageStudio() {
        if !runControlAction("open-image-studio") {
            openURL(routedURL(state.localURL, extraItems: [
                URLQueryItem(name: "intent", value: "image-studio"),
                URLQueryItem(name: "view", value: "studio"),
            ]))
        }
    }
    ...
    @objc private func launchArcade() {
        guard runControlAction("launch-arcade") else { return }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            self?.enterChromeFullScreen()
        }
    }
    ...
    private func runControlAction(_ action: String) -> Bool {
        guard let binaryPath = controlBinaryPath() else { return false }
        let process = Process()
    ...
    private func routedURL(_ urlString: String?, extraItems: [URLQueryItem]) -> String? {
        guard let urlString, var components = URLComponents(string: urlString) else { return urlString }
    ...
    private func enterChromeFullScreen() {
        let script = """
        ...
        tell application "System Events"
          tell process "Google Chrome"
            try
              click first menu item of menu 1 of menu bar item "View" of menu bar 1 whose name is "Enter Full Screen"
              return "fullscreen"
            end try
          end tell
        end tell
    ...
    private func controlBinaryPath() -> String? {
        if let authoritative = authoritativeBinaryPath() {
            return authoritative
        }
        return state.binaryPath
    }

    private func authoritativeBinaryPath() -> String? {
        guard
            let repoPath = state.repoPath?.trimmingCharacters(in: .whitespacesAndNewlines),
            !repoPath.isEmpty
        else {
            return nil
        }
        let bundleURL = FileManager.default.homeDirectoryForCurrentUser
            ...
            .appendingPathComponent("Alcove.app", isDirectory: true)
        let repoMarkerURL = bundleURL
            .appendingPathComponent("Contents", isDirectory: true)
            .appendingPathComponent("Resources", isDirectory: true)
            .appendingPathComponent("repo-path")
        guard
            let repoText = try? String(contentsOf: repoMarkerURL, encoding: .utf8).trimmingCharacters(in: .whitespacesAndNewlines),
            !repoText.isEmpty,
            URL(fileURLWithPath: repoText).standardizedFileURL.path == URL(fileURLWithPath: repoPath).standardizedFileURL.path
        else {
            return nil
        }
