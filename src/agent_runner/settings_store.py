...
from .providers import infer_provider_for_model
...
def load_app_settings(path: Path, defaults: AppSettings) -> AppSettings:
    ...
    arcade_repo_raw = raw.get("arcade_repo_path")
    arcade_repo_path = defaults.arcade_repo_path
    if isinstance(arcade_repo_raw, str) and arcade_repo_raw.strip():
        arcade_repo_path = Path(arcade_repo_raw.strip())
    legacy_model = _coerce_text(raw.get("model"), defaults.model) or defaults.model
    openai_model = _optional_text(raw.get("openai_model"), defaults.openai_model)
    if not openai_model and _looks_like_provider_model(legacy_model, ProviderKind.CODEX, defaults.provider):
        openai_model = legacy_model
    openai_model = openai_model or defaults.openai_model or "gpt-5.3-codex"
    open_source_model = _optional_text(raw.get("open_source_model"), defaults.open_source_model)
    if not open_source_model and _looks_like_provider_model(legacy_model, ProviderKind.OLLAMA, defaults.provider):
        open_source_model = legacy_model
    resolved_model = openai_model if provider == ProviderKind.CODEX else (open_source_model or legacy_model)
    return AppSettings(
        provider=provider,
        ...
        arcade_repo_path=arcade_repo_path,
...
def save_app_settings(path: Path, settings: AppSettings) -> None:
    ...
    if settings.arcade_repo_path is not None:
        payload["arcade_repo_path"] = str(settings.arcade_repo_path)
...
def _looks_like_provider_model(model: str, provider: ProviderKind, default_provider: ProviderKind) -> bool:
    text = str(model or "").strip()
    if not text:
        return False
    return infer_provider_for_model(text, default_provider) == provider
