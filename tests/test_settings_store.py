...
def test_save_and_load_settings_round_trip(tmp_path: Path) -> None:
    ...
    assert loaded.openai_model == "gpt-5.4"
    assert loaded.open_source_model == "llama3.2"
    assert loaded.vision_model == "qwen3.5:9b"
...
def test_load_settings_falls_back_when_missing(tmp_path: Path) -> None:
    ...
    assert AppSettings().phase_timeout_seconds == 7200
...
def test_load_settings_ignores_invalid_types_and_preserves_defaults(tmp_path: Path) -> None:
    ...
    assert loaded.openai_model == "gpt-5.4"
    assert loaded.open_source_model == "qwen3.5:9b"
...
def test_load_settings_migrates_legacy_model_into_family_defaults(tmp_path: Path) -> None:
    path = tmp_path / ".agent-runner" / "app-settings.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        """
    ...
    assert loaded.provider == ProviderKind.OLLAMA
    assert loaded.model == "qwen3.5:9b"
    assert loaded.openai_model == "gpt-5.3-codex"
    assert loaded.open_source_model == "qwen3.5:9b"
