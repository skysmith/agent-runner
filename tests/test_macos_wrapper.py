...
def test_open_browser_for_state_routes_image_studio_intent(monkeypatch, tmp_path: Path) -> None:
    save_wrapper_state(
        tmp_path,
        {
            "server_info": {"local_url": "http://127.0.0.1:8765"},
    ...
    assert result == "http://127.0.0.1:8765?intent=image-studio&view=studio"
    assert opened == [result]
